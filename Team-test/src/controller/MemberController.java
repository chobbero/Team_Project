...
import action.member.UserJoinProAction;
...
public class MemberController extends HttpServlet {
    ...
    protected void doProcess(HttpServletRequest request, HttpServletResponse response)
        ...
        if (command.equals("/UserJoinForm.mb")) {
            ...
            // 단, 글 삭제를 위한 게시물 번호(board_num)와 글 삭제 후 페이징 처리를 위한 페이지번호(page)를 가져와서
            // request.setAttribute() 메서드로 저장하여 이동
        ...
        if (forward != null) {
            if (forward.isRedirect()) { // true 일 경우 redirect 방식으로 포워딩
                ...
                // => jsp 페이지로 바로 이동하지 않고, 특정 Action 클래스로 이동하기 위해 isRedirect 값을 true 로 변경하여
                // Redirect 방식으로 포워딩 => *.bo 로 이동
